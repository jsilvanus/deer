...
export class ChatSession {
    ...
    append(msg) {
        if (!msg || typeof msg !== 'object' || !msg.role)
            throw new Error('append requires a message with a role');
        const entry = { role: msg.role, content: String(msg.content ?? '') };
        if (msg.toolName)
            entry.toolName = msg.toolName;
        if (msg.toolCalls)
            entry.toolCalls = Array.isArray(msg.toolCalls) ? msg.toolCalls : [];
    ...
    async send(userMessage, { executeTool, maxIterations = 10, maxTokens = 512 } = {}) {
        ...
        throw new Error(`ChatSession: agentic loop exceeded maximum iterations (${maxIterations}) without producing a final answer`);
    }
    async destroy() {
