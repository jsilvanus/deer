...
function _extractMessagesFromSession(sessionLike) {
    if (!sessionLike)
        return [];
    if (Array.isArray(sessionLike.messages))
        return sessionLike.messages;
    if (Array.isArray(sessionLike.history))
        return sessionLike.history;
    if (typeof sessionLike.history === 'function')
        return sessionLike.history();
    if (Array.isArray(sessionLike._history))
        return sessionLike._history;
    return [];
}
