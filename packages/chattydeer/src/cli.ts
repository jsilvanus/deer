...
async function main() {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    else if (a === '--token' || a === '-t') { token = args[++i] ?? token; }
    else if (a === '--system' || a === '-s') { systemPrompt = args[++i] ?? systemPrompt; }
    else if (a === '--provider' || a === '-p') { provider = args[++i] ?? provider; }
    else if (a === '--device' || a === '-d') { device = args[++i] ?? device; }
  ...
  rl.on('close', async () => {
    try { await session.destroy(); } catch (err) { /* ignore */ }
    process.exit(0);
  });
}
