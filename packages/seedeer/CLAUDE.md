...
- **Image Embeddings** — Two model families: joint image-text space (CLIP/SigLIP) or vision-only (DINOv3) for cross-modal or image-to-image search
...
- `src/detector.js` — Object detection with RT-DETR-class models
...
- `src/visual-embedder.js` — Vision-only embeddings (DINOv3)
